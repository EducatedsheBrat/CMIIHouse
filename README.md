...
**Naming:** the app is *Den Points* (*CMII Den Points* where the institution needs naming), and that name stays the same every year. "House" is still the word for the four groups: Lumina, Doron, Asé and Kaizen. Each season has its own journey name. **LeaderQuest** is the 2026-2027 journey. A season's `name` is the journey and its id is the academic year, so the app shows them together as "LeaderQuest · 2026–2027". Admins name or rename a season under **Admin → Seasons**.
...
Demo sign-in on the **emulators only** (email/password), all with password `DenPoints-demo-2026`:
